...
/// <c>ChessArmy.Core</c> (mêmes règles que le jeu) avant d'écrire le JSON.
/// </summary>
internal sealed class MainForm : Form
{
    ...
    private Button TilePaletteButton(TileInfo tile)
    {
        var btn = new Button
        {
            ...
            Text = tile.Key, ImageAlign = ContentAlignment.TopCenter,
        };
    ...
    /// </summary>
    private void DropTileAtCursor(string movedKey)
    {
        if (_catalog is null)
            return;
        var target = _palette.GetChildAtPoint(_palette.PointToClient(Cursor.Position)) as Button;
        if (target?.Tag is not string targetKey || _catalog.TileForKey(targetKey) is null || targetKey == movedKey)
            return;
    ...
    {
        var key = ch.ToString();   // pinceau spawn/objet = 1 caractère, stocké en string comme les clés de tuile
        var btn = new Button
        {
            Size = new Size(96, 44), Margin = new Padding(4), Tag = key, FlatStyle = FlatStyle.Flat,
            BackColor = color, ForeColor = Color.Black, Text = $"{ch}  {label}",
            TextAlign = ContentAlignment.MiddleCenter,
        };
        btn.FlatAppearance.BorderColor = Color.FromArgb(90, 92, 100);
        btn.Click += (_, _) => { _canvas.Brush = key; SelectPaletteButton(btn); };
    ...
    {
        foreach (Control c in _palette.Controls)
            if (c is Button b && b.Tag is string s && s == tile.Key)
            {
    ...
    private void Open()
    {
        using var dlg = new OpenFileDialog
        {
            ...
            Filter = "Maps Chess Army (*.json)|*.json",
