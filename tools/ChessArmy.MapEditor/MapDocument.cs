...
internal sealed class MapDocument
{
    ...
    /// <summary>Terrain : chaque case est une CLÉ de tuile de 1 OU 2 caractères (cf. TileRenderCatalog.KeyLeads).
    /// Les autres calques restent 1 caractère par case.</summary>
    ...
    public static MapDocument Load(string path, IReadOnlySet<char> tileLeads)
    {
        ...
        FillTiles(doc.Tiles, dto.Tiles, dto.Width, dto.Height, tileLeads);
    ...
    /// <summary>Remplit le calque terrain en TOKENISANT chaque ligne : une case fait 2 caractères si elle
    /// commence par un préfixe (<paramref name="leads"/>), sinon 1. Case manquante = espace (rendue en
    /// placeholder). Tolérant (l'éditeur n'est pas le validateur : la sauvegarde re-valide via Core).</summary>
    private static void FillTiles(string[,] grid, List<string>? rows, int width, int height, IReadOnlySet<char> leads)
    {
        for (var r = 0; r < height; r++)
        {
            var line = rows is not null && r < rows.Count ? rows[r] : "";
            ...
            for (var c = 0; c < width; c++)
            {
                if (i >= line.Length) { grid[r, c] = " "; continue; }
                var len = leads.Contains(line[i]) && i + 1 < line.Length ? 2 : 1;
                grid[r, c] = line.Substring(i, len);
                i += len;
            }
        }
    }
    ...
    public void Resize(int newWidth, int newHeight, string defaultTileKey)
    {
        var t = new string[newHeight, newWidth];
    ...
    /// <summary>Sérialise le calque terrain en concaténant les clés (1 ou 2 caractères) de chaque case. Une
    /// case vide/non initialisée est écrite comme un espace pour garder l'alignement des colonnes.</summary>
    private List<string> ToRowsTiles(string[,] grid)
    {
        var rows = new List<string>(Height);
        for (var r = 0; r < Height; r++)
        {
            ...
            for (var c = 0; c < Width; c++)
                sb.Append(string.IsNullOrEmpty(grid[r, c]) ? " " : grid[r, c]);
